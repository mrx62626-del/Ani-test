...
export function AnimeProvider({ children }: { children: ReactNode }) {
    ...
    const HOME_CACHE_PREFIX = 'yorumi_home_cache_v3';
    ...
    const fetchHomeData = async () => {
        ...
        const fastBundlePromise = anilistService.getHomeFastData()
            .then((fast) => {
        ...
        const fetchSpotlight = async () => {
            ...
            try {
                ...
                for (const delay of retryDelays) {
                    ...
                    try {
                        const { data } = await anilistService.getSpotlightAnime();
        ...
        const fetchTrending = async () => {
            ...
            try {
                const tData = await anilistService.getTrendingAnime(1, 10);
                if (tData?.data) {
        ...
        const fetchLatestUpdates = async () => {
            ...
            try {
                const latestData = await anilistService.getLatestUpdates();
                if (latestData?.data) {
        ...
        const fetchPopular = async () => {
            ...
            try {
                const pData = await anilistService.getPopularThisSeason(1, 10);
                if (pData?.data) {
        ...
        const fetchPopularMonth = async () => {
            ...
            try {
                const pData = await anilistService.getPopularThisMonth(1, 10);
                if (pData?.data) {
    ...
    useEffect(() => {
        const fetchPageData = async () => {
            ...
            try {
                ...
                const data = await anilistService.getTopAnime(currentPage);
                if (data?.data) {
    ...
    const resolveAndCacheEpisodes = async (anime: Anime): Promise<{ session: string | null, eps: Episode[] }> => {
            ...
            try {
                ...
                for (let index = 0; index < queryList.length; index += 3) {
                    const results = await Promise.all(
                        queryList.slice(index, index + 3).map(q => lunapaheService.searchAnime(q).then(res => res || []).catch(() => []))
                    );
        ...
        if (session) {
            ...
            try {
                const epData =
				await lunapaheService.getEpisodes(
					session
				);
    ...
    const handleAnimeClick = async (anime: Anime) => {
        ...
        try {
            ...
            if (detailsData?.data) {
                ...
                if (anime.title) {
                    try {
                        const search = await anilistService.searchAnime(anime.title, 1);
    ...
    const prefetchPage = (page: number) => {
        if (page <= lastVisiblePage) {
            anilistService.getTopAnime(page);
        }
    };
    ...
    const fetchViewAll = async (type: 'latest' | 'trending' | 'seasonal' | 'continue_watching' | 'popular', page: number) => {
        ...
        try {
            let data;
            if (type === 'latest') data = await anilistService.getLatestUpdatesPage(page);
            else if (type === 'trending') data = await anilistService.getTrendingAnime(page, 18);
            else if (type === 'seasonal') data = await anilistService.getPopularThisSeason(page, 18);
            else if (type === 'popular') data = await anilistService.getTopAnime(page); // Re-use getTopAnime for "View All" pagination
